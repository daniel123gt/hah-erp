...
import { Plus, User, Scan, DollarSign, FileText, Loader2, UserPlus } from "lucide-react";
import type { Patient } from "~/services/patientsService";

const RX_ECOGRAFIA_TYPES = [
  { value: "rx", label: "RX" },
...
interface AddRxEcografiaRecordModalProps {
  onCreated: () => void;
}

export function AddRxEcografiaRecordModal({ onCreated }: AddRxEcografiaRecordModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    setLoadingData(true);
    const department = getDepartmentForCategory("rx_ecografias");
    Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await rxEcografiaRecordsService.create({
        fecha: form.fecha,
      ...
      toast.success("Registro de RX/Ecografía creado");
      resetForm();
      setOpen(false);
      onCreated();
    } catch (err) {
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Registro manual de RX o ecografía (fecha, paciente, tipo, técnico/médico, ingreso y costo).
            </DialogDescription>
          </DialogHeader>
          {loadingData ? (
                ...
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Combobox
                      ...
                      placeholder={loadingData ? "Cargando..." : "Buscar técnico o médico"}
      ...
      <CreatePatientSubmodal
        ...
        description="Se seleccionará automáticamente para el registro de RX/Ecografía."
      />
    </>
  );
}
