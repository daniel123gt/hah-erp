...
import { rxEcografiaRecordsService, type RxEcografiaReportResult } from "~/services/rxEcografiaRecordsService";
...
export default function ReportesRegistroRxEcografias() {
  ...
  const [report, setReport] = useState<RxEcografiaReportResult | null>(null);
  ...
  type ChartTabId = "distribucion" | "evolucion" | "tipo" | "registros";
  ...
  const CHART_COLORS = ["#0d9488", "#2563eb", "#16a34a", "#dc2626", "#ea580c", "#7c3aed", "#db2777", "#64748b"];

  const handleExportCSV = () => {
    if (exportingCSV) return;
    setExportingCSV(true);
    try {
      const headers = "Fecha,Paciente,Tipo,Técnico/Médico,Método,Ingreso (S/.),Costo (S/.),Utilidad (S/.)\n";
      ...
      link.href = url;
      link.download = `reporte-rx-ecografias-${startDate}-${endDate}.csv`;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <Button variant="outline" onClick={() => navigate("/registro-rx-ecografias")}>
              <ArrowLeft className="w-4 h-4 mr-2" />
              Volver
            </Button>
            <h1 className="text-2xl font-bold text-gray-900">Reportes - Registro RX / Ecografías</h1>
          </div>
          ...
          <div className="flex border-b border-gray-200 gap-0">
            {(
              [
                ...
                { id: "tipo" as ChartTabId, label: "Composición por tipo" },
            ...
            <CardContent className="pt-6">
                  ...
                  {chartDataByDate.length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={chartDataByDate} margin={{ top: 10, right: 20, left: 0, bottom: 5 }}>
                        ...
                        <Tooltip formatter={(v: number) => [`S/ ${v.toFixed(2)}`, ""]} labelFormatter={(_, payload) => payload?.[0]?.payload?.label} />
                        <Legend />
                        <Area type="monotone" dataKey="ingreso" name="Ingresos" stroke="#0d9488" fill="#0d9488" fillOpacity={0.3} />
              ...
              {chartTab === "tipo" && (
                <div className="h-[320px] flex items-center justify-center">
                  {tipoStats.length === 0 ? (
                    <p className="text-gray-500 text-center py-12">No hay datos por tipo en el rango.</p>
                  ) : (
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={tipoStats.slice(0, 8).map((p) => ({
                  ...
                  {chartDataTopRecords.length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={chartDataTopRecords} layout="vertical" margin={{ top: 5, right: 30, left: 80, bottom: 5 }}>
                        ...
                        <Legend />
                        <Bar dataKey="utilidad" name="Utilidad (S/.)" fill="#0d9488" radius={[0, 4, 4, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  )}
                </div>
