...
import { rxEcografiaRecordsService, type RxEcografiaRecord } from "~/services/rxEcografiaRecordsService";
...
export default function RegistroRxEcografiasDashboard() {
  const navigate = useNavigate();
  const [todayRecords, setTodayRecords] = useState<RxEcografiaRecord[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      rxEcografiaRecordsService.list({ fromDate: today, toDate: today, limit: 100 }),
      rxEcografiaRecordsService.list({ fromDate: from, toDate: to, limit: 500 }),
    ])
  ...
  const cards = [
    {
      title: "Listado",
      description: "Ver y editar registros de RX/Ecografías completadas (ingreso, costo, notas)",
      icon: <List className="w-8 h-8 text-teal-500" />,
      action: () => navigate("/registro-rx-ecografias/listado"),
    ...
    },
    {
      ...
      action: () => navigate("/registro-rx-ecografias/reportes"),
      ...
      <Card className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
            ...
            Registros RX/Ecografías de hoy
          </h2>
          {todayRecords.length > 0 && (
            <Button variant="outline" size="sm" onClick={() => navigate("/registro-rx-ecografias/listado")}>
              Ver listado
            </Button>
          )}
        </div>
        {loading ? (
          ...
          <p className="text-gray-500 text-center py-6">No hay registros de RX/Ecografías para hoy.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left py-2 font-medium">Técnico / Médico</th>
